...
User answers 5 symptom questions + 2 free-text questions  (the "Intake")
  ...
  the answers (see §2)           instead (see §4)
      ...
      - a few evidence-based insights, each with a citation and a
        personalized "what this means for you" line (see §2.1)
      - a few "what might help" recommendations, each backed by
        either a research citation or a community stat (see §2.6)
      - up to 3 supplement suggestions, kept in their own area (§2.7)
      - one short overview per dominant symptom, used when the user
        clicks a symptom on the radar chart (see §2.8)
                         │
                         ▼
...
> You are a careful, evidence-based PMOS (polyendocrine metabolic ovarian
> syndrome, formerly known as PCOS, renamed in 2026) symptom triage
> assistant. You never diagnose. You only cite from the provided citation
> list by id, never invent studies or links. Every insight's citationIds
> must come from the reference list below. Symptom keys in dominantSymptoms
> and insights[].symptom must be chosen from the provided symptom key list
> exactly as written.
>
> *(followed by the full list of symptom keys and the full list of allowed
> citations, plus the rules for `meaning`, `recommendations`, `supplements`,
> and `symptomOverviews` described below, see §3 for what the reference
...
- *"You only cite from the provided citation list... never invent"*: this is
  ...
  hallucinating a fake study. It can only ever point to the real citations
...
| `insights` | 3-5 evidence statements, each tagged with which citation(s) back it up, plus a `meaning` line personalized to this user (§2.1) |
| `recommendations` | 2-5 "what might help" suggestions, each grounded in research or community data, never invented (§2.6) |
| `supplements` | 0-3 dietary supplement suggestions, kept separate with a safety disclaimer (§2.7) |
| `symptomOverviews` | One short synthesis per dominant symptom, shown when that symptom is clicked on the radar chart (§2.8) |
...
## 2.1. Insight `meaning`: personalized, not generic

Each insight has both a `statement` (the research fact itself, e.g. "Irregular
ovulation is one of the three core diagnostic criteria for PMOS...") and a
`meaning`: 1-2 sentences that interpret that fact for *this specific person*,
referencing their own answers (their severity rating for that symptom, or
something they mentioned in free text).

`meaning` is explicitly not allowed to introduce a new research claim,
citation, or study; it can only personalize the `statement` that's already
grounded. The prompt also tells Claude to vary its phrasing and avoid
defaulting to a generic closing line like "raise this with a clinician" on
every single insight, since that caveat is already covered once by
`confidenceNote`. The fallback scorer (§4) mirrors this by combining the
user's actual severity rating with the symptom's description, so the
"What this means for you" box is genuinely tailored even without AI.
...
[`src/lib/research-data.ts`](src/lib/research-data.ts) (plus any ingested
PDFs, via [`src/lib/pdf-sources.ts`](src/lib/pdf-sources.ts)) and pastes them
as plain text into the system prompt. Claude literally cannot see anything
beyond what we paste in for that one request; there's no wider internet or
paper archive it's pulling from.
...
```ts
const c = ALL_CITATIONS[id];   // look the id up in OUR merged dictionary, not Claude's answer
if (!c) return null;           // if it's not a real entry, show nothing
```

`ALL_CITATIONS` (in `src/lib/pdf-sources.ts`) merges our hand-curated
`CITATIONS` with every ingested PDF's citation metadata, it is entirely our
own data. Claude has no way to add to it or change it. So even in the
unlikely event Claude *did* invent an id like `"smith2024"`, the lookup
simply returns nothing, and that citation badge silently doesn't render.
There is no code path that can turn an AI-invented id into a clickable link
or a fake DOI. Worst case, an insight shows up with no source badge next to
it; it can never show a *wrong* one.
...
- **`source: "research"`**: a suggestion we've written ourselves, grounded
  ...
  depression in PMOS"*, backed by the Jiskoot 2022 meta-analysis). These live
...
This is why every recommendation on the Profile page sits in a clearly
labelled card, "What might help" (research-backed, with a clickable source)
or "What women with PMOS are trying" (community, with a real % and its own
basis), there is no third, ungrounded kind.
...
## 2.8. The symptom radar chart and per-symptom overviews

The Profile page opens with a spider/radar chart
([`src/components/symptom-radar-chart.tsx`](src/components/symptom-radar-chart.tsx))
plotting the user's own severity ratings (0-4) for each symptom they
answered. It's clickable: tapping a vertex (or its label) focuses the whole
page on just that symptom, tapping the centre returns to the full view.

When a symptom is selected, the "chart overview" panel switches to show that
symptom's `symptomOverviews` entry: a 1-2 sentence synthesis built only from
the insights/recommendations/supplements already produced for that symptom
elsewhere in the same response. Like `meaning` (§2.1), the prompt explicitly
forbids introducing any new claim or citation here, it's a summary, not a
new fact. Selecting a symptom also filters the "What science says",
"What might help", "Supplements", and community-comparison sections down to
just that symptom.
...
one file: [`src/lib/research-data.ts`](src/lib/research-data.ts), plus any
PDFs ingested into `src/lib/pdf-sources.ts` (§3.5). Nothing on the Profile
page is made up on the spot, it always traces back to an entry in one of
these.

### A) The citation library (`CITATIONS`)

A list of real, published PMOS/PCOS studies and guidelines. Each one looks
like this:
...
> reflect real, well-known PMOS/PCOS research themes, but the `summary` text
> is our own paraphrase, not a verbatim quote. Before using this beyond a
> demo, each entry should be checked against the actual paper by someone with
> literature-review experience.

### A.1) Evidence level grading (`EvidenceLevel`)

Every citation carries an optional `evidenceLevel`: `"strong"`, `"moderate"`,
`"limited"`, or `"low"`. This is **our own editorial classification** based
on study design (guidelines/Cochrane reviews/meta-analyses = strong;
narrative reviews or single studies = moderate; small or herbal trials =
limited; patient-information pages = low), not a grade the papers assign
themselves. It drives the colored badge next to each research insight and
the "strongest evidence first" sort on the Profile page. The plain-language
legend shown in the "How we grade evidence" info button (`EVIDENCE_INFO` in
`research-data.ts`) explains each level to the user in confidence-level
terms rather than research jargon.
...
that show up on the Profile page, and optionally `recommendations` and
`supplements` lists.
...
```ts
{
  ...
  citations: ["teede2018", "legro2013"],   // just ids, the full citation is looked up from CITATIONS/ALL_CITATIONS
  insights: [
    "Irregular ovulation is one of the three core diagnostic criteria for PMOS...",
  ],
},
```
...
Six PDFs are currently ingested: `jiskoot2022`, `barber2021`, `han2024`,
`moinijazani2019`, `nhs2026medicines`, and `nice2026` (the source of the
2026 PMOS rename).
...
- `COMMUNITY_BASELINES`: e.g. "78% of community members also report
  irregular periods"
- `COMMUNITY_STATS`: headline numbers like average months to diagnosis, used
  on both the landing page and the Profile page's community comparison card
- `FORUM_POSTS`: sample forum posts with fake authors, used on the
  Community page
- `COMMUNITY_RECOMMENDATIONS`: what other members report trying, each
  traceable back to specific forum posts (see §2.6)

---

## 3.5. Using real PDFs as a data source

We can drop in an actual research PDF and have Claude quote/paraphrase its
real content, not just our hand-written summary of it, while keeping the
exact same "can't fake a citation" guarantee from §2.5. Here's how it works,
end to end.
...
file, add it to the `PDF_DOCS` list, and add its `evidenceLevel` to
`PDF_EVIDENCE_LEVELS`. That's the only code change needed, everything
downstream (citation rendering, prompt injection) picks it up automatically.
...
**Where the limits are:** this keyword-matching approach works well for a
handful of PDFs. If we ingest many more papers, keyword matching will start
missing relevant passages that use different wording than the user's
answers. At that point it's worth upgrading `findRelevantChunks` to use real
semantic search (embeddings + similarity, e.g. the AI SDK's `embed`) instead
of keyword counting.
...
3. We add up (frequency × weight) across all symptoms, turn that into a
   5-95% score, and pick the top 2-4 scoring symptoms as "dominant."
4. For each dominant symptom, we pull its first pre-written insight and
   citations straight out of the dataset in §3, and build a personalized
   `meaning` line from the user's actual severity rating plus the symptom's
   description (no generation happening, just a lookup and a template).
5. We also build `symptomOverviews` (one per dominant symptom, referencing
   the severity rating) and pull any matching research/community
   recommendations and supplements for those symptoms.
...
authors/year, journal, link, a short summary in your own words, and an
`evidenceLevel` (§3.A.1).
...
### To change the Vera theme/branding
Open [`src/app/globals.css`](src/app/globals.css) for the coral/orange color
palette (light and dark mode), [`src/components/nav.tsx`](src/components/nav.tsx)
for the logo/wordmark, and [`src/app/layout.tsx`](src/app/layout.tsx) for the
...
| `src/lib/scoring.ts` | The answer template (schema) + fallback scoring formula + fallback insights/recommendations/supplements/overviews |
| `src/lib/research-data.ts` | **The dataset**: citations + evidence levels + symptoms + insights + research-backed recommendations + supplements |
...
| `src/lib/pdf-sources.ts` | Registers ingested PDFs, merges their citations into `ALL_CITATIONS`, does the keyword search, grades PDF evidence levels |
...
| `src/lib/storage.ts` | Saves the user's profile/check-ins on their own device (localStorage) |
| `src/app/page.tsx` | The Vera landing page |
| `src/app/intake/page.tsx` | The question-by-question intake screen |
| `src/app/profile/page.tsx` | The results screen: radar chart, click-to-filter, insights, recommendations, supplements, community |
| `src/components/symptom-radar-chart.tsx` | The interactive, clickable severity radar chart on the Profile page |
| `src/app/checkin/page.tsx` | The weekly check-in screen |
| `src/app/community/page.tsx` | The mock forum screen |
| `src/components/nav.tsx` | The site nav + Vera logo |
| `src/app/globals.css` | The Vera coral/orange theme (light + dark) |
